                ...
                <table class="min-w-full divide-y divide-gray-100">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left text-xs font-bold text-gray-400 uppercase tracking-wider">Name</th>
                            <th class="px-4 py-3 text-left text-xs font-bold text-gray-400 uppercase tracking-wider">Slug</th>
                            <th class="px-4 py-3 text-left text-xs font-bold text-gray-400 uppercase tracking-wider">Blogs</th>
                    ...
                    <tbody class="divide-y divide-gray-50">
                        @forelse($categories as $category)
                            <tr class="hover:bg-gray-50 transition">
                                ...
                                <td class="px-4 py-3">
                                    <code class="text-xs bg-blue-50 text-blue-700 px-2 py-1 rounded">{{ $category->slug }}</code>
                                </td>
                                <td class="px-4 py-3 text-sm text-gray-600">
                                    <span class="inline-flex items-center justify-center bg-gray-100 text-gray-700 text-xs font-semibold rounded-full w-7 h-7">
                                ...
                                <td class="px-4 py-3">
                                    <div class="flex items-center gap-2">
                                        <a href="{{ route('blog-categories.show', $category->slug) }}" target="_blank"
                                        ...
                                        <form action="{{ route('blog-categories.destroy', $category->id) }}" method="POST"
                                              onsubmit="return confirm('Delete this category? Blogs in it will become uncategorized.')">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
